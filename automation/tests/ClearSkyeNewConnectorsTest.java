...
public class ClearSkyeNewConnectorsTest extends AppTest {
	...
	protected void executeSuccessCase(AppPage appPage, TestDataBean tdb) {		
		...
		Reporter.log("Navigating to the 'Connectors' tab.");
		connector.clickOnConnectorsTab();
		
		connector.switchToIframe();
		webDriverUtil.waitAWhile();
		
		Reporter.log("Validating the 'Create a Connector' process.");
		connector.createAConnector(tdb);
		
		webDriverUtil.waitAWhile();
		connector.switchToContent();
