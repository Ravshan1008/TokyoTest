...
import com.clearskye.test.actions.ClearSkyeRequestVariableSettings;
...
public class ClearSkyeRequestVariableSettingsTest extends AppTest {
	...
	protected void executeSuccessCase(AppPage appPage, TestDataBean tdb) {		
		...
		ClearSkyeRequestVariableSettings request = new ClearSkyeRequestVariableSettings(webDriverUtil);

		Reporter.log("Validating the 'User Login' process.");
		user.login();
		
		Reporter.log("Navigating to the 'Review Campaigns' tab.");
		request.clickOnReviewVariableSettingsTab();
		
		request.switchToIframe();
		webDriverUtil.waitAWhile();
		
		Reporter.log("Validating the 'campaign review' process.");
		request.createANewRequest(tdb);
		
		webDriverUtil.waitAWhile();
		request.switchToContent();
